...
static int jtag_state; // default: test_logic_reset
...
int handle_data(int fd)
{
	...
	do {
		...
		char cmd[16];
		// unit:bytes, buffer[2048 * 2] bytes for tms vector and tdi vector
		...
		if (memcmp(cmd, "ge", 2) == 0) {
			...
			if (send(fd, result, 4, 0) != 4) { // todo: set tck is not work yet
				perror("write");
				return 1;
			}
			if (verbose) {
		...
		// Read length (in bits) in command shift, fixed 4 bytes
		int len;
		if (sread(fd, &len, 4) != 1) {
		...
		// near_bytes or number_of_bytes = ceil(len_bits / 8)
		...
		if (sread(fd, buffer, nr_bytes * 2) != 1) { // tms vector and tdi vector
		...
		if ((jtag_state == exit1_ir && len == 5 && buffer[0] == 0x17) || (jtag_state == exit1_dr && len == 4 && buffer[0] == 0x0b)) {
			...
			for (i = 0; i < len; ++i) { // len_bits
				// Do the actual cycle.
				int tms = !!(buffer[i/8] & (1<<(i&7))); // tms is 0 or 1
				// Track the state.
	...
	while (1) {
		...
		for (fd = 0; fd <= maxfd; ++fd) {
			if (FD_ISSET(fd, &read)) {
				...
				} else if (handle_data(fd)) {// Otherwise, do work.
			...
			} else if (FD_ISSET(fd, &except)) { // Abort connection?
				if (verbose)
